...
export default class Trivia extends Component {
  ...
  next = () => {
    if (this.qno < quiz.length - 1) {
      ...
      this.props.getScore(this.score * 100 / (quiz.length))
    }
  }
        ...
        <SafeAreaView style={{ backgroundColor: '#1d4c7f', paddingTop: 10 }}>
          <View style={styles.container}>
            ...
            <View style={{ flex: 1, flexDirection: 'column', justifyContent: "space-between", alignItems: 'center' }}>
              ...
              <View>
                {Object.keys(currentOptions).map((item, i) => (<View key={i} style={{ margin: 10 }}>

                  <Button countCheck={this.state.countCheck} onColor={"skyblue"} onPress={(status) => this.props.answerQuestion(status, item)} title='this is option button'>
