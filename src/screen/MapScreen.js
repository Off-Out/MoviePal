import React, { Component } from 'react';
import { View, StyleSheet } from 'react-native';
import { Location, Permissions } from 'expo';
import { Button } from 'react-native-elements';
import get from 'lodash/get';
import pick from 'lodash/pick';

import YelpService from '../services/yelp';
// import { Map } from '../component';

class MapScreen extends Component {
	filterButtons = [
		{ label: 'Open now', color: '#9C27B0', filter: { openNow: true } },

		{
			label: 'Walking Distance',
			color: '#00BCD4',
			filter: { radius: 3000 }
		}

	];

	state = {
		location: null,
		errorMessage: null,
		movieTheaters: []
	};

	componentWillMount() {
		this.getLocationAsync();
	}

	getMovieTheater = async filter => {
		const coords = get(this.state.location, 'coords');
		const userLocation = pick(coords, ['latitude', 'longitude']);
		let movieTheaters = await YelpService.getMovieTheater(
			userLocation,
			filter
		);
		this.setState({ movieTheaters });
	};

	getLocationAsync = async () => {
		let { status } = await Permissions.askAsync(Permissions.LOCATION);
		if (status !== 'granted') {
			this.setState({
				errorMessage: 'Permission to access location was denied'
			});
		}

		let location = await Location.getCurrentPositionAsync({});
		await this.setState({ location });
		this.getMovieTheater();
	};

	handleFilterPress = filter => {
		this.getMovieTheater(filter);
	};

	renderFilterButtons() {
		return this.filterButtons.map((button, i) => (
			<Button
				key={i}
				title={button.label}
				buttonStyle={{
					backgroundColor: button.color,
					...styles.button
				}}
				onPress={() => this.handleFilterPress(button.filter)}
			/>
		));
	}

	render() {
		const { location, movieTheaters } = this.state;

		return (
			<View style={{ flex: 7 }}>
				<Map location={location} places={movieTheaters} />
				<View style={styles.filters}>{this.renderFilterButtons()}</View>
			</View>
		);
	}
}

const styles = {
	filters: {
		flexDirection: 'row',
		justifyContent: 'flex-start',
		alignItems: 'center',
		flexWrap: 'wrap'
	},
	button: {
		marginVertical: 4
	}
};

export { MapScreen };



// import React, { Component } from 'react';
// import {StyleSheet, View, Text } from 'react-native';
// import MapView , { PROVIDER_GOOGLE } from 'react-native-maps';

//     const dummyPlace =[{name:'My Thai', category: 'Restaurants' , latitude:41.8758 , longitude: -87.6189 },
// {name:'My Yoga', category: 'Activities' , latitude:41.8836 , longitude:-87.6163 },
// {name:'Aquarium', category: 'Sport' , latitude:41.8776 , longitude:-87.6140 },
// {name:'Market', category: 'Art' , latitude:41.8923 , longitude:-87.6167 }]

// export default class MapScreen extends Component {

//   render() {


//     return (
//       <View style={styles.container}>
//      <MapView
//         provider={PROVIDER_GOOGLE}
//         style={styles.map}
//         region={{
//           latitude: 41.8781,
//           longitude: -87.6298,
//           latitudeDelta: 0.0922,
//           longitudeDelta: 0.0421

//         }}
//       >

//       {/* <MapView.Marker coordinate={{
//           latitude: 41.8781,
//           longitude: -87.6298,}}
//           title={'Are you there'}
//           description={'Yes I am!!'}/> */}
//       {dummyPlace.map( place =>(
//         <MapView.Marker key={place.name} coordinate={{
//           latitude: place.latitude,
//           longitude: place.longitude
//         }}
//          title={place.category}
//           description={place.name}

//         />
//       ))}
//       </MapView>
//       </View>
//     );
//   }
// }

// const styles= StyleSheet.create({
//   container:{
//     position: 'absolute',
//     top:0,
//     left:0,
//     bottom:0,
//     right:0,
//     justifyContent: 'flex-end',
//     alignItems: 'center'
//   },
//   map:{
//     position: 'absolute',
//     top:0,
//     left:0,
//     bottom:0,
//     right:0,
//   }
// })
